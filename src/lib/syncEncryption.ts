...
const SYNC_ITERATIONS = 600000;
const SYNC_KEY_SIZE = 256 / 32; // AES-256

const CHANNEL_SALT = 'privdo-channel-id';
const SYNC_KEY_SALT = 'privdo-sync-key';

export function generatePassphrase(wordCount = 12): string {
  const words: string[] = [];
  for (let i = 0; i < wordCount; i++) {
    const randomBytes = CryptoJS.lib.WordArray.random(2);
    const index = randomBytes.words[0] >>> 16; // Use top 16 bits
...
export function encryptForSync(data: unknown, syncKey: string): string {
  ...
  return CryptoJS.AES.encrypt(jsonStr, syncKey).toString();
}

export function decryptFromSync(cipherText: string, syncKey: string): unknown | null {
  if (!cipherText) return null;
  try {
    const bytes = CryptoJS.AES.decrypt(cipherText, syncKey);
...
const WORDLIST: string[] = [
  ...
  'enable','encourage','end','endless','endorse','enemy','energy','enforce','engage','engine',
  ...
  'hen','hero','hip','hire','history','hobby','hockey','hold','hole','holiday',
  ...
  'industry','infant','inflict','inform','initial','inject','inmate','inner','innocent','input',
  ...
  'length','lens','leopard','lesson','letter','level','liberty','library','license','life',
  ...
  'surprise','surround','survey','suspect','sustain','swallow','swamp','swap','swarm','sweet',
  ...
  'trumpet','trust','truth','try','tube','tuna','tunnel','turkey','turn','turtle',
  'twelve','twenty','twice','twin','twist','two','type','typical','ugly','umbrella',
  'unable','unaware','uncle','uncover','under','undo','unfair','unfold','unhappy','uniform',
  'union','unique','unit','universe','unknown','unlock','until','unusual','unveil','update',
  'upgrade','uphold','upon','upper','upset','urban','usage','use','used','useful',
