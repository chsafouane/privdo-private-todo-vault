...
export function encryptForSync(data: unknown, syncKey: string): string {
  ...
  const key = CryptoJS.enc.Hex.parse(syncKey);
  ...
  const hmacKey = deriveHmacKey(syncKey);
...
export function decryptFromSync(cipherText: string, syncKey: string): unknown | null {
  ...
  try {
    ...
    if (parsed.v === FORMAT_VERSION && parsed.iv && parsed.ct && parsed.mac) {
      const hmacKey = deriveHmacKey(syncKey);
  ...
  // Legacy: passphrase-mode decryption (backward compatibility)
  try {
